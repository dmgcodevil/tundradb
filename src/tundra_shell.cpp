...
public:
    ...
    antlrcpp::Any visitCreateSchemaStatement(tundraql::TundraQLParser::CreateSchemaStatementContext *ctx) override {
        ...
        if (!result.ok()) {
            throw std::runtime_error("Failed to add schema: " + result.status().ToString());
        }
    ...
    antlrcpp::Any visitCreateNodeStatement(tundraql::TundraQLParser::CreateNodeStatementContext *ctx) override {
        ...
        for (auto prop : propList->propertyAssignment()) {
            ...
            std::cout << "DEBUG: Raw property " << prop_name << "=" << prop_value << std::endl;
        ...
        for (const auto& field : schema->fields()) {
            ...
            std::cout << "DEBUG: Converting field " << field_name 
                      << " (type=" << field_type->ToString() 
                      << ") from value '" << value_str << "'" << std::endl;
            ...
            if (field_type->id() == arrow::Type::STRING) {
                ...
                if (!status.ok()) {
                    throw std::runtime_error("Failed to finish string array: " + status.ToString());
                }
                ...
                try {
                    ...
                    bool is_numeric = !cleaned_value.empty() && 
                                     cleaned_value.find_first_not_of("0123456789") == std::string::npos;
                    if (!is_numeric) {
                    ...
                    if (!status.ok()) {
                        throw std::runtime_error("Failed to append int64: " + status.ToString());
                    }
                    std::shared_ptr<arrow::Array> array;
                    status = builder.Finish(&array);
                    if (!status.ok()) {
                        throw std::runtime_error("Failed to finish int64 array: " + status.ToString());
                    }
                    data[field_name] = array;
                } catch (const std::exception& e) {
                    throw std::runtime_error("Error converting '" + value_str + "' to int64: " + e.what());
                }
                ...
                try {
                    ...
                    bool is_numeric = !cleaned_value.empty() && 
                                    cleaned_value.find_first_not_of("0123456789.+-eE") == std::string::npos;
                    if (!is_numeric) {
                        throw std::runtime_error("Value contains invalid characters for a floating-point number");
                    }
                    
                    double double_value = std::stod(cleaned_value);
                    std::cout << "DEBUG: Converted double value: " << double_value << std::endl;
                    ...
                    status = builder.Finish(&array);
                    if (!status.ok()) {
                        throw std::runtime_error("Failed to finish double array: " + status.ToString());
                    }
                    data[field_name] = array;
                } catch (const std::exception& e) {
                    throw std::runtime_error("Error converting '" + value_str + "' to double: " + e.what());
                }
            }
        }
    ...
    antlrcpp::Any visitCreateEdgeStatement(tundraql::TundraQLParser::CreateEdgeStatementContext *ctx) override {
    ...
    antlrcpp::Any visitMatchStatement(tundraql::TundraQLParser::MatchStatementContext *ctx) override {
        ...
        for (size_t i = 0; i < edges.size(); i++) {
            ...
            if (outgoing) {
                query_builder.traverse(node_alias, edge_type, target_alias + ":" + target_type, traverse_type);
            } else {
                // For incoming edges, swap source and target
                query_builder.traverse(target_alias + ":" + target_type, edge_type, node_alias, traverse_type);
            }
        }
        ...
        if (ctx->selectClause()) {
            ...
            for (auto field : selectClause->selectField()) {
                ...
                if (field->IDENTIFIER().size() > 1) {
                    ...
                    column_name = field->IDENTIFIER(0)->getText() + "." + field->IDENTIFIER(1)->getText();
                } else {
                    // Just table prefix: u
...
void printTableAsAscii(const std::shared_ptr<arrow::Table>& table) {
    ...
    for (size_t i = 0; i < column_names.size(); i++) {
        ...
        std::cout << std::string(left_pad, ' ') << name << std::string(right_pad, ' ') << "|";
    }
    std::cout << "\n";
    ...
    for (int64_t row = 0; row < table->num_rows(); row++) {
        ...
        for (int col = 0; col < table->num_columns(); col++) {
            std::string value = tundradb::stringifyArrowScalar(table->column(col), row);
            size_t padding = column_widths[col] - value.length();
            size_t left_pad = padding / 2;
            size_t right_pad = padding - left_pad;
            std::cout << std::string(left_pad, ' ') << value << std::string(right_pad, ' ') << "|";
        }
        std::cout << "\n";
...
int main() {
    ...
    std::string db_path = "tundra_shell_db_" + std::to_string(tundradb::now_millis());
