...
/// Temporary feature flag for the experimental binding-based materializer.
///
/// Keep this disabled by default so query execution uses the legacy row builder
/// until the binding path covers the full join/WHERE matrix and can replace the
/// old implementation outright.
static constexpr bool kEnableBindingMaterialization = false;

/// One partially-materialized result row plus the alias bindings that produced
/// it.
///
/// `row` stores the actual field values accumulated so far.
/// `node_ids` records node alias -> bound node id, or `nullopt` for a LEFT
/// null-extension.
/// `edge_ids` records edge alias -> bound edge id, or `nullopt` when the edge
/// side of a LEFT traversal is absent.
struct BindingRow {
  std::shared_ptr<Row> row;
  std::unordered_map<std::string, std::optional<int64_t>> node_ids;
  std::unordered_map<std::string, std::optional<int64_t>> edge_ids;
};

/// Returns true when every traversal in the query can use the new
/// binding-based materializer.
///
/// At the moment that path handles INNER and LEFT joins directly. RIGHT and
/// FULL still fall back to the legacy BFS/schema-based builder.
bool supports_binding_materialization(const std::vector<Traverse>& traverses) {
  if (!kEnableBindingMaterialization) {
    return false;
  }
  return std::all_of(traverses.begin(), traverses.end(), [](const auto& t) {
    return t.traverse_type() == TraverseType::Inner ||
           t.traverse_type() == TraverseType::Left;
  });
}

/// Deep-copy a binding row so one input row can branch into multiple output
/// rows when a traversal fans out to several matches.
auto clone_binding_row(const BindingRow& binding) -> BindingRow {
  BindingRow copy;
  copy.row = std::make_shared<Row>(*binding.row);
  copy.node_ids = binding.node_ids;
  copy.edge_ids = binding.edge_ids;
  return copy;
}

/// Materialize one bound node alias into the row's cell storage.
///
/// If the alias has projected fields in the final schema, this loads the node
/// by id and writes its values into the row. Otherwise it is a cheap no-op.
auto fill_node_cells(const SchemaRef& ref, int64_t node_id, BindingRow& binding,
                     const QueryState& query_state) -> arrow::Status {
  const auto idx_it = query_state.schema_field_indices().find(ref.value());
  if (idx_it == query_state.schema_field_indices().end()) {
    return arrow::Status::OK();
  }

  ARROW_ASSIGN_OR_RAISE(const auto schema_name,
                        query_state.resolve_schema(ref));
  ARROW_ASSIGN_OR_RAISE(const auto node, query_state.node_manager->get_node(
                                             schema_name, node_id));
  binding.row->set_cell_from_node(idx_it->second, node,
                                  query_state.temporal_context.get());
  return arrow::Status::OK();
}

/// Materialize one bound edge alias into the row's cell storage.
///
/// If the edge alias contributes projected fields to the final schema, this
/// loads the edge by id and writes its properties into the row. Otherwise it is
/// a cheap no-op.
auto fill_edge_cells(const std::string& edge_alias, int64_t edge_id,
                     BindingRow& binding, const QueryState& query_state)
    -> arrow::Status {
  ...
  ARROW_ASSIGN_OR_RAISE(const auto edge, query_state.edge_store->get(edge_id));
  binding.row->set_cell_from_edge(idx_it->second, edge, edge_schema->fields(),
                                  query_state.temporal_context.get());
  return arrow::Status::OK();
}

/// Bind one node alias for the current row.
///
/// Behavior:
/// - if the alias is unbound, record the supplied id (or NULL binding)
/// - if the alias is already bound, succeed only when the new id matches
/// - when a concrete node id is accepted, materialize that node's fields into
///   the row immediately
auto bind_node_alias(BindingRow& binding, const SchemaRef& ref,
                     const std::optional<int64_t>& node_id,
                     const QueryState& query_state) -> arrow::Result<bool> {
  const auto [it, inserted] =
      binding.node_ids.try_emplace(ref.value(), node_id);
  if (!inserted) {
  ...
  ARROW_RETURN_NOT_OK(fill_node_cells(ref, *node_id, binding, query_state));
  binding.row->id = binding.row->id >= 0 ? binding.row->id : *node_id;
  return true;
}

/// Bind one edge alias for the current row.
///
/// This mirrors `bind_node_alias(...)`, but for optional edge aliases attached
/// to a TRAVERSE. Accepted concrete edge ids are materialized into the row
/// immediately so later stages do not need to revisit the edge store.
auto bind_edge_alias(BindingRow& binding, const std::string& edge_alias,
                     const std::optional<int64_t>& edge_id,
                     const QueryState& query_state) -> arrow::Result<bool> {
  const auto [it, inserted] = binding.edge_ids.try_emplace(edge_alias, edge_id);
  if (!inserted) {
    return it->second == edge_id;
  }
...
/// Returns true when a recorded graph connection belongs to the given TRAVERSE
/// clause.
///
/// Matching checks the full traverse identity: source alias, target alias,
/// edge type, and optional edge alias.
bool connection_matches_traverse(const GraphConnection& conn,
                                 const Traverse& traverse) {
  return conn.source.value() == traverse.source().value() &&
         conn.target.value() == traverse.target().value() &&
         conn.edge_type == traverse.edge_type() &&
         conn.edge_alias == traverse.edge_alias();
}

/// Return the currently valid connections for one bound row traversing one
/// TRAVERSE clause.
///
/// Starting from the row's already-bound source alias, this filters the stored
/// graph connections down to those that:
/// - match the exact TRAVERSE clause
/// - still point to a live target id in `query_state.ids()[target_alias]`
///
/// The second rule lets alias-local filtering shrink the acceptable target set
/// before row expansion runs.
auto get_live_connections_for_traverse(const BindingRow& binding,
                                       const Traverse& traverse,
                                       const QueryState& query_state)
    -> llvm::SmallVector<GraphConnection, 4> {
  llvm::SmallVector<GraphConnection, 4> matches;

  const auto source_it = binding.node_ids.find(traverse.source().value());
  if (source_it == binding.node_ids.end() || !source_it->second.has_value()) {
    return matches;
  }

  const int64_t source_id = *source_it->second;
  if (!query_state.connections().contains(traverse.source().value())) {
    return matches;
  }

  const auto& by_source =
      query_state.connections().at(traverse.source().value());
  if (!by_source.contains(source_id)) {
    return matches;
  }

  const auto target_ids_it = query_state.ids().find(traverse.target().value());
  if (target_ids_it == query_state.ids().end()) {
    return matches;
  }

  const auto& target_ids = target_ids_it->second;
  for (const auto& conn : by_source.at(source_id)) {
    if (!connection_matches_traverse(conn, traverse)) {
      continue;
    }
    if (!target_ids.contains(conn.target_id)) {
      continue;
    }
...
/// Expand one bound row through a single TRAVERSE clause.
///
/// Semantics:
/// - `INNER`: one output row per surviving connection; zero connections drops
///   the row
/// - `LEFT`: one output row per surviving connection; zero connections keeps
///   the row and binds the introduced target/edge aliases to NULL
///
/// This is the core of the binding-based materialization model: join behavior
/// is applied exactly where the TRAVERSE is processed instead of being inferred
/// later from graph connectivity.
auto expand_traverse_binding(const BindingRow& binding,
                             const Traverse& traverse,
                             const QueryState& query_state)
    -> arrow::Result<std::vector<BindingRow>> {
  std::vector<BindingRow> expanded_rows;
  auto live_connections =
      get_live_connections_for_traverse(binding, traverse, query_state);
  ...
  if (live_connections.empty()) {
    ...
    auto null_extended = clone_binding_row(binding);
    ARROW_ASSIGN_OR_RAISE(const bool target_ok,
                          bind_node_alias(null_extended, traverse.target(),
                                          std::nullopt, query_state));
    if (!target_ok) {
      return expanded_rows;
    }
    if (traverse.edge_alias().has_value()) {
      ARROW_ASSIGN_OR_RAISE(
          const bool edge_ok,
          bind_edge_alias(null_extended, traverse.edge_alias().value(),
                          std::nullopt, query_state));
      if (!edge_ok) {
        return expanded_rows;
      }
    }
    expanded_rows.push_back(std::move(null_extended));
    return expanded_rows;
  }

  expanded_rows.reserve(live_connections.size());
  for (const auto& conn : live_connections) {
    auto next = clone_binding_row(binding);
    ARROW_ASSIGN_OR_RAISE(
        const bool target_ok,
        bind_node_alias(next, traverse.target(), conn.target_id, query_state));
    if (!target_ok) {
      continue;
    }
    if (traverse.edge_alias().has_value()) {
      ARROW_ASSIGN_OR_RAISE(const bool edge_ok,
                            bind_edge_alias(next, traverse.edge_alias().value(),
                                            conn.edge_id, query_state));
      if (!edge_ok) {
        continue;
      }
    }
    expanded_rows.push_back(std::move(next));
  }
  return expanded_rows;
}

/// Build result rows by expanding bound aliases in clause order.
///
/// This keeps join semantics local to each TRAVERSE:
/// - INNER drops rows with no matching connection.
/// - LEFT preserves the existing row and null-extends the new target/edge.
///
/// Unlike the legacy BFS/schema-based builder, this does not infer join
/// behavior from missing target IDs after the fact.
auto populate_rows_by_bindings(
  ...
  const auto root_ids_it = query_state.ids().find(query_state.root.value());
  if (root_ids_it == query_state.ids().end()) {
    return rows;
  }

  std::vector<BindingRow> bindings;
  bindings.reserve(root_ids_it->second.size());
  for (const auto root_id : root_ids_it->second) {
    BindingRow binding{.row = std::make_shared<Row>(
                           create_empty_row_from_schema(output_schema))};
    ARROW_ASSIGN_OR_RAISE(
        const bool root_ok,
        bind_node_alias(binding, query_state.root, root_id, query_state));
    if (!root_ok) {
      continue;
    }
  ...
  for (const auto& traverse : traverses) {
    std::vector<BindingRow> next_bindings;
    for (const auto& binding : bindings) {
      ARROW_ASSIGN_OR_RAISE(auto expanded, expand_traverse_binding(
                                               binding, traverse, query_state));
      next_bindings.insert(next_bindings.end(),
  ...
  for (auto& binding : bindings) {
    binding.row->id = row_id++;
    rows->push_back(std::move(binding.row));
  }
  return rows;
}
...
arrow::Result<std::shared_ptr<arrow::Table>> Database::build_result_table(
  ...
  if (supports_binding_materialization(query_state.traversals)) {
    ARROW_ASSIGN_OR_RAISE(
        rows, populate_rows_by_bindings(query_state, query_state.traversals,
                                        output_schema));
  } else {
    ARROW_ASSIGN_OR_RAISE(rows,
