...
namespace tundradb {
    ...
    static arrow::Result<std::shared_ptr<arrow::Table>> create_table(
        ...
        for (const auto& field : schema->fields()) {
            switch (field->type()->id()) {
                ...
                case arrow::Type::STRING:
                    ...
                    break;
                default:
                    return arrow::Status::NotImplemented("Unsupported type: ", field->type()->ToString());
            }
        }
        
        // Process nodes in chunks
        std::vector<std::shared_ptr<arrow::Array>> arrays_for_current_chunk;
        std::vector<std::vector<std::shared_ptr<arrow::Array>>> chunks_per_field(schema->num_fields());
        ...
        for (const auto& node : nodes) {
                    ...
                    if (array->length() == 0 || array->IsNull(0)) {
                        ...
                        switch (field->type()->id()) {
                            case arrow::Type::INT64: {
                                ...
                                ARROW_RETURN_NOT_OK(
                                    static_cast<arrow::Int64Builder*>(builders[i].get())->Append(int_array->Value(0)));
                                break;
                            }
                            case arrow::Type::STRING: {
                                auto str_array = std::static_pointer_cast<arrow::StringArray>(array);
                                ARROW_RETURN_NOT_OK(
                                    static_cast<arrow::StringBuilder*>(builders[i].get())->Append(str_array->GetString(0)));
                                break;
                            }
                            default:
                                return arrow::Status::NotImplemented("Unsupported type: ", field->type()->ToString());
            ...
            // If we've reached the chunk size, finalize the arrays and reset the builders
        ...
        for (int i = 0; i < schema->num_fields(); i++) {
            chunked_arrays.push_back(std::make_shared<arrow::ChunkedArray>(chunks_per_field[i]));
        }
    ...
    template <typename T>
    T ValueOrDieWithContext(const arrow::Result<T>& result, 
