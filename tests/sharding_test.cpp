...
TEST_F(ShardingTest, UpdateNodes) {
  ...
  auto update_result = db->update_node("test-schema", node->id, "name",
                                       Value{"Updated"}, UpdateType::SET);
...
TEST_F(ShardingTest, Compaction) {
  ...
  auto update_result = db->update_node("test-schema", 1, "name",
                                       Value{"Removed"}, UpdateType::SET);
  ASSERT_TRUE(update_result.ok());
