...
protected:
    ...
    void TearDown() override {
        ...
        std::cout << "Peak memory usage: " << memory_tracker_.get_peak_memory_kb() << " KB" << std::endl;
        std::cout << "Memory increase: " << memory_tracker_.get_memory_increase_kb() << " KB" << std::endl;
    }
    ...
    void run_concurrent_test(tundradb::RCUConcurrentSet<T>& set,
        ...
        auto insert_worker = [&](int worker_id) {
            ...
            std::uniform_real_distribution<double> dist(0.0, 1.0); // Create distribution
            ...
            while (true) {
                ...
                if (set.insert(value)) {
                    ...
                    if (dist(rng) < delete_ratio) { // Fixed usage of random generator
        ...
        auto delete_worker = [&](int worker_id) {
            ...
            while (keep_reading.load() || !values_to_delete.empty()) {
                T value_to_delete;
                {
                    std::lock_guard<std::mutex> lock(delete_mutex);
                    ...
                    if (values_to_delete.size() > 1) {
                        std::uniform_int_distribution<size_t> idx_dist(0, values_to_delete.size() - 1);
        ...
        for (int i = 0; i < num_insert_threads; i++) {
            insert_futures.push_back(std::async(std::launch::async, insert_worker, i));
        }
        ...
        for (int i = 0; i < num_delete_threads; i++) {
            delete_futures.push_back(std::async(std::launch::async, delete_worker, i));
        }
...
class RCUConcurrentSetParamTest : public RCUConcurrentSetTest,
...
TEST_P(RCUConcurrentSetParamTest, ConcurrentOperations) {
    ...
    run_concurrent_test(set, config.insert_threads, config.delete_threads,
                       config.read_threads, config.elements, config.delete_ratio);
