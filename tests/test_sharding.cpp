...
protected:
    ...
    void SetUp() override {
        ...
        ASSERT_TRUE(result.ok()) << "Failed to register schema: " << result.status().ToString();
    }
    
    // Create a test node with specified name and count
    std::shared_ptr<Node> create_test_node(const std::string& name, int64_t count) {
        ...
        EXPECT_TRUE(result.ok()) << "Failed to create node: " << result.status().ToString();
...
TEST_F(ShardingTest, UpdateNodes) {
    ...
    ASSERT_TRUE(update_result.ok()) << "Failed to update node: " << update_result.status().ToString();
...
TEST_F(ShardingTest, Compaction) {
    ...
    ASSERT_TRUE(compact_result.ok()) << "Failed to compact: " << compact_result.status().ToString();
    ...
    ASSERT_TRUE(compact_all_result.ok()) << "Failed to compact all schemas: " << compact_all_result.status().ToString();
}
